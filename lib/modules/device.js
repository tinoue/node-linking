...
LinkingDevice.prototype.connect = function () {
	let promise = new Promise((resolve, reject) => {
		if (this.connected === true) {
			...
			// local patch not to throw error if it has been already conncted.
			resolve();
			return;
		}
		...
		p.connect((error) => {
			if (error) {
				// Workaround for connection state mismatch.
			    	if (error.message && error.message.match(/Peripheral already connected/)) {
					// fall through. Threat this as success.
				} else {
