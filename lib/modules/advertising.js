...
LinkingAdvertising.prototype.parse = function(peripheral) {
	...
	if(version >= 4) {
		// Version should be less than 4 by spec. If not, this might be encrypted data
		let data = manu.slice(2); // data begins after the version field.

		// shift whole buffer to left by 4 bits
		for(let offset=0; offset<data.length-1; offset++) {
			data.writeUInt8((data.readUInt8(offset) << 4) & 0xf0 | (data.readUInt8(offset+1) >>> 4), offset);
		}
